...
public class JMSProfileUpdateValidator {
  ...
  public static Validation validate(UpdateRequest request) throws Exception {
    ...
    if (jmsProfileDTO.getId() == null || jmsProfileDTO.getId() == 0) {
      validation.isValid(false);
      validation.setErrorCode(MessageUtil.getMessage("invalid.input.field.zero.code"));
      validation.setErrorMessage(
          MessageUtil.getMessage("invalid.input.field.zero", "JMSProfileID"));
    ...
    if (StringUtils.isBlank(jmsProfileDTO.getProviderURL())) {
      ...
      validation.setErrorCode(MessageUtil.getMessage("invalid.input.field.blank.code"));
      validation.setErrorMessage(
          MessageUtil.getMessage("invalid.input.field.blank", "Provider URL"));
    ...
    if (StringUtils.isBlank(jmsProfileDTO.getInitialContextFactory())) {
      ...
      validation.setErrorCode(MessageUtil.getMessage("invalid.input.field.blank.code"));
      validation.setErrorMessage(
          MessageUtil.getMessage("invalid.input.field.blank", "Initial context factory"));
    ...
    if (StringUtils.isBlank(jmsProfileDTO.getServiceName())) {
      ...
      validation.setErrorCode(MessageUtil.getMessage("invalid.input.field.blank.code"));
      validation.setErrorMessage(
          MessageUtil.getMessage("invalid.input.field.blank", "Service name"));
    ...
    if (jmsProfileDTO.getConnectionRetryInterval() == null || jmsProfileDTO.getConnectionRetryInterval() < 5) {
      ...
      validation.setErrorMessage(
          MessageUtil.getMessage("invalid.input.field.minvalue", "Connection retry interval", "5"));
