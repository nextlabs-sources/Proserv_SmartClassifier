...
public class ReportUpdateService extends Service {
	
	public ReportUpdateService(ServletContext servletContext) {
		super(servletContext);
	}
	
	public ReportUpdateService(ServletContext servletContext, ServletRequest servletRequest, ServletResponse servletResponse) {
		super(servletContext, servletRequest, servletResponse);
	}
	
	public ServiceResponse updateReport(UpdateRequest updateRequest) {
		...
		try {
			...
			Report reportEntity = ((ReportDTO)updateRequest.getData()).getEntity();
			reportEntity.setModifiedBy(getUserName());
			new ReportManager(getSessionFactory(), session).updateReport(reportEntity);
			...
			response.setMessage(MessageUtil.getMessage("no.entity.found.modify", "rule"));
