				...
				function($timeout, $compile) {
					return {
						...
						link : function(scope, element, attrs, ngModel) {
							...
							var numericNgModelName = attrs.ngModel
									|| "numericModel";
							...
							var useDecimals = numberFormat ? (numberFormat
									.toLowerCase() === "c" ? true : false)
									: false;
							...
							var init = function() {
								angular
										.forEach(
												...
												function(property) {
													if (angular
															.isDefined(attrs[property])) {
														options[property] = parseNumber(
																attrs[property],
																useDecimals);
													} else if (options
															.hasOwnProperty(property)
															&& angular
																	.isUndefined(options[property])) {
														options[property] = 0;
													}
												});
							...
							angular.forEach(properties, function(property) {
								...
								attrs.$observe(property, function(newVal) {
									if (!!newVal) {
										init();
										numericElement.spinner('option',
												property, parseNumber(newVal,
														useDecimals));
									}
								});
							});
							...
							attrs
									.$observe(
											...
											function(newVal) {
												...
												init();
												var newValState = angular.element
														.type(newVal) === "boolean" ? newVal
														: newVal.toLowerCase() == 'true';
												numericElement.spinner(
														'option', 'disabled',
														newValState);
											});
							// destroy jquery spinner object when directive
							// object is destroyed
							function destroy() {
