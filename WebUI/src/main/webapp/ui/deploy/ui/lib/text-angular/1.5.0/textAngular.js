...
(function() { // encapsulate all variables so they don't become global vars
	...
	// this is set true when a blur occurs as the blur of the ta-bind triggers
	// before the click
	...
	if (_browserDetect.webkit) {
		document.addEventListener("mousedown", function(_event) {
			...
			if (globalContentEditableBlur && curelement !== null) {
				...
				if (!isEditable) {
					document.getElementById('textAngular-editableFix-010203040506070809').setSelectionRange(0, 0); // set
																													// caret
																													// focus
																													// to
																													// an
					...
					if (curelement.select) {
						curelement.select(); // use select to place cursor
												// for input elements.
					}
				}
			}
	...
	/*
	 * Custom stylesheet for the placeholders rules. Credit to:
	...
	if (_browserDetect.ie > 8 || _browserDetect.ie === undefined) {
		...
		if (!sheet) {
			...
			sheet = (function() {
				...
				// Add the <style> element to the page, add as first so the
				// styles can be overridden by custom stylesheets
		...
		_addCSSRule = function(_sheet, selector, rules) {
			var insertIndex;
			var insertedRule;
			// This order is important as IE 11 has both cssRules and rules but
			// they have different lengths - cssRules is correct, rules gives an
			// error in IE 11
	...
	angular.module('textAngular.factories', []).factory('taBrowserTag', [function() {
		...
		return function(val) {
			...
			angular.forEach(taCustomRenderers, function(renderer) {
				var elements = [];
				// get elements based on what is defined. If both defined do
				// secondary filter in the forEach after using selector string
				if (renderer.selector && renderer.selector !== '')
					elements = element.find(renderer.selector);
				/*
				 * istanbul ignore else: shouldn't fire, if it does we're
				 * ignoring everything
				 */
		...
		var taFixChrome = function(html) {
			...
			// only replace when something has changed, else we get focus
			// problems on inserting lists
			if (lastIndex > 0) {
		...
		function wrapNested(html, wrapTag) {
			...
			return wrapTag + html.substring(0, lastIndex) +
			// get the start tags reversed - this is safe as we construct the
			// strings with no content except the tags
			angular.element(wrapTag)[0].outerHTML.substring(wrapTag.length) + html.substring(lastIndex);
		}
		...
		function transformLegacyStyles(html) {
			...
			while (match = styleElementMatch.exec(html)) {
				...
				if (angular.isString(styleVal) && styleRegex.test(styleVal)) {
					...
					// recursively find more legacy styles in html before this
					// tag and after the previous match (if any)
		...
		return function taSanitize(unsafe, oldsafe, ignore) {
			// unsafe html should NEVER built into a DOM object via
			// angular.element. This allows XSS to be inserted and run.
			if (!ignore) {
				try {
			...
			// any exceptions (lets say, color for example) should be made here
			// but with great care
			...
			try {
				...
				// do this afterwards, then the $sanitizer should still throw
				// for bad markup
				if (ignore)
					safe = unsafe;
			} catch (e) {
			...
			// Do processing for <pre> tags, removing tabs and return carriages
			// outside of them
		...
		return function(editor) {
			...
			// pass into the action the deferred function and also the function
			// to reload the current selection if rangy available
			var result;
			try {
				result = this.action(deferred, _editor.startAction());
				// We set the .finally callback here to make sure it doesn't get
				// executed before any other .then callback.
			...
			if (result || result === undefined) {
				// if true or undefined is returned then the action has
				// finished. Otherwise the deferred action will be resolved
				// manually.
				deferred.resolve();
			}
		};
				...
				if (selectedElement !== undefined) {
					...
					if (command.toLowerCase() === 'insertorderedlist' || command.toLowerCase() === 'insertunorderedlist') {
						...
						if (tagName === selfTag) {
							...
							// if it's one of those block elements we have to
							// change the contents
							// if it's a ol/ul we are changing from one to the
							// other
							...
							// if we get here then all the contents of the
							// ta-bind are selected
							_nodes = taSelection.getOnlySelectedElements();
							if (_nodes.length === 0) {
								// here is if there is only text in ta-bind ie
								// <div ta-bind>test content</div>
								...
								for (i = 0; i < _nodes.length; i++) {
									/*
									 * istanbul ignore else: catch for
									 * real-world can't make it occur in testing
									 */
									if (_nodes[i].nodeType !== 3) {
										var $n = angular.element(_nodes[i]);
										/*
										 * istanbul ignore if: browser check
										 * only, phantomjs doesn't return
										 * children nodes but chrome at least
										 * does
										 */
										if (_nodes[i].tagName.toLowerCase() === 'li')
											continue;
										else if (_nodes[i].tagName.toLowerCase() === 'ol' || _nodes[i].tagName.toLowerCase() === 'ul') {
											html += $n[0].innerHTML; // if
																		// it's
																		// a
																		// list,
																		// add
											...
											html += $n[0].childNodes[0].innerHTML; // if
																					// it's
																					// a
																					// list,
																					// add
						...
						if (tagName === optionsTagName) {
							...
							for (i = 0; i < contents.length; i++) {
								/*
								 * istanbul ignore next: can't test - some wierd
								 * thing with how phantomjs works
								 */
							...
							if (_nodes.length === 0)
								_nodes = [$target[0]];
							// find the parent block element if any of the nodes
							// are inline or text
							for (i = 0; i < _nodes.length; i++) {
							...
							if (angular.element(_nodes[0]).hasClass('ta-bind')) {
								...
								// regular block elements replace other block
								// elements
								for (i = 0; i < _nodes.length; i++) {
									$target = angular.element(options);
						...
						if (_selection.collapsed) {
							// insert text at selection, then select then just
							// let normal exec-command run
	...
	/*
	 * istanbul ignore next: all browser specifics and PhantomJS dosen't seem to
	 * support half of it
	 */
	function($window, $document, taDOM) {
		...
		var brException = function(element, offset) {
			/*
			 * check if selection is a BR element at the beginning of a
			 * container. If so, get the parentNode instead. offset should be
			 * zero in this case. Otherwise, return the original element.
			 */
		...
		var api = {
			getSelection : function() {
				...
				// Check if the container is a text node and return its parent
				// if so
			...
			// topNode is the contenteditable normally, all manipulation MUST be
			// inside this.
			insertHtml : function(html, topNode) {
				...
				if (children.length > 0) {
					...
					// check for blockelements - if they exist then we have to
					// split the current element in half (and all others up to
					// the closest block element) and insert all children
					// in-between.
					// If there are no block elements, or there is a mixture we
					// need to create textNodes for the non wrapped text (we
					// don't want them spans messing up the picture).
					nodes = [];
					for (_childI = 0; _childI < children.length; _childI++) {
						if (!((children[_childI].nodeName.toLowerCase() === 'p' && children[_childI].innerHTML.trim() === '') || // empty
																																	// p
																																	// element
						(children[_childI].nodeType === 3 && children[_childI].nodeValue.trim() === '') // empty
																										// text
																										// node
						)) {
				...
				if (isInline) {
					...
					if (range.collapsed && range.startContainer !== topNode) {
						if (range.startContainer.innerHTML && range.startContainer.innerHTML.match(/^<[^>]*>$/i)) {
							// this log is to catch when innerHTML is something
							// like `<img ...>`
							...
							// split element into 2 and insert block element in
							// middle
							if (range.startContainer.nodeType === 3 && range.startContainer.parentNode !== topNode) { // if
																														// text
																														// node
								parent = range.startContainer.parentNode;
								secondParent = parent.cloneNode();
								// split the nodes into two lists - before and
								// after, splitting the node with the selection
								// into 2 text nodes.
								...
								while (!VALIDELEMENTS.test(parent.nodeName)) {
									...
									secondParent = parent.cloneNode();
									// split the nodes into two lists - before
									// and after, splitting the node with the
									// selection into 2 text nodes.
		...
		var taDOM = {
			// recursive function that returns an array of angular.elements that
			// have the passed attribute set on them
			...
			splitNodes : function(nodes, target1, target2, splitNode, subSplitIndex, splitIndex) {
				...
				while (nodes.length > 0 && (isNaN(splitIndex) || splitIndex !== index) && nodes[0] !== splitNode) {
					startNodes.appendChild(nodes[0]); // this removes from the
														// nodes array (if
														// proper childNodes
														// object.
					index++;
	...
	angular.module('textAngular.taBind', ['textAngular.factories', 'textAngular.DOM']).service('_taBlankTest', [function() {
		...
		return function(_defaultTest) {
			return function(_blankVal) {
				if (!_blankVal)
					return true;
				// find first non-tag match - ie start of string or after tag
				// that is not whitespace
				var _firstMatch = /(^[^<]|>)[^<]/i.exec(_blankVal);
				var _firstTagIndex;
				if (!_firstMatch) {
					// find the end of the first tag removing all the
					// Don't do a global replace as that would be waaayy too
					// long, just replace the first 4 occurences should be
					// enough
				...
				if (/^[^<>]+$/i.test(_blankVal))
					return false;
				// this regex is to match any number of whitespace only between
				// two tags
				if (_blankVal.length === 0 || _blankVal === _defaultTest || /^>(\s|&nbsp;)*<\/[^>]+>$/ig.test(_blankVal))
					return true;
				// this regex tests if there is a tag followed by some optional
				// whitespace and some text after that
		...
		return {
			link : function(scope, element, attrs) {
				...
				element.on('mousedown', function(e, eventData) {
					/*
					 * istanbul ignore else: this is for catching the jqLite
					 * testing
					 */
					if (eventData)
						angular.extend(e, eventData);
					// this prevents focusout from firing on the editor when
		...
		// OR any non-form element with contenteditable="contenteditable"
		// ta-bind="html|text" ng-model
		return {
			...
			link : function(scope, element, attrs, controller) {
				...
				// the option to use taBind on an input or textarea is required
				// as it will sanitize all input into it correctly.
				...
				var _lastKey;
				// see http://www.javascripter.net/faq/keycodes.htm for good
				// information
				...
				// Left arrow, Up arrow, Right arrow, Down arrow, Insert,
				// Delete,
				// f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12
				// NumLock, ScrollLock
				var BLOCKED_KEYS = /^(9|19|20|27|33|34|35|36|37|38|39|40|45|112|113|114|115|116|117|118|119|120|121|122|123|144|145)$/i;
				// UNDO_TRIGGER_KEYS - spaces, enter, delete, backspace, all
				// punctuation
				// Backspace, Enter, Space, Delete, (; :) (Firefox), (= +)
				// (Firefox),
				// Numpad +, Numpad -, (; :), (= +),
				// (, <), (- _), (. >), (/ ?), (` ~), ([ {), (\ |), (] }), (' ")
				// NOTE - Firefox: 173 = (- _) -- adding this to
				// UNDO_TRIGGER_KEYS
				var UNDO_TRIGGER_KEYS = /^(8|13|32|46|59|61|107|109|173|186|187|188|189|190|191|192|219|220|221|222)$/i;
				var _pasteHandler;

				// defaults to the paragraph element, but we need the line-break
				// or it doesn't allow you to type into the empty element
				// non IE is '<p><br/></p>', ie is '<p></p>' as for once IE gets
				// it correct...
				...
				// mappings is an array of maps from events to specialKeys as
				// declared in textAngularSetup
				var _keyMappings = [
				// ctrl/command + z
				{
					specialKey : 'UndoKey',
				...
				var _ensureContentWrapped = function(value) {
					...
					// console.log('domTest.children().length():',
					// domTest.children().length);
						...
						if (!_foundBlockElement) {
							...
							for (i = 0; i < _children.length; i++) {
								...
								if (nodeName === '#comment') {
									...
									// determine if this is all whitespace, if
									...
									if (!text.trim()) {
										...
										// <p>...</p> or whatever
										// attrs.taDefaultWrap is set to.
										value += "<" + attrs.taDefaultWrap + ">" + text + "</" + attrs.taDefaultWrap + ">";
									}
								} else if (!nodeName.match(BLOCKELEMENTS)) {
									/*
									 * istanbul ignore next: Doesn't seem to
									 * trigger on tests
									 */
									var _subVal = (node.outerHTML || node.nodeValue);
									/*
									 * istanbul ignore else: Doesn't seem to
									 * trigger on tests, is tested though
									 */
									if (_subVal.trim() !== '')
				...
				var _undo = scope['$undoTaBind' + (attrs.id || '')] = function() {
					/*
					 * istanbul ignore else: can't really test it due to all
					 * changes being ignored as well in readonly
					 */
				...
				// in here we are undoing the converts used elsewhere to prevent
				// the < > and & being displayed when they shouldn't in the
				// code.
				...
				var _setViewValue = function(_val, triggerUndo, skipRender) {
					...
					if (typeof triggerUndo === "undefined" || triggerUndo === null)
						triggerUndo = true && _isContentEditable; // if not
																	// contentEditable
																	// then the
																	// native
																	// undo/redo
					...
					if (typeof _val === "undefined" || _val === null)
						_val = _compileHtml();
					if (_blankTest(_val)) {
						// this avoids us from tripping the ng-pristine flag if
						// we click in and out with out typing
				...
				// parsers trigger from the above keyup function or any other
				// time that the viewValue is updated and parses it for storage
				// in the ngModel
				ngModel.$parsers.push(_sanitize);
				ngModel.$parsers.unshift(_ensureContentWrapped);
				// because textAngular is bi-directional (which is awesome) we
				// need to also sanitize values going in from the server
				...
				// this code is used to update the models when data is
				// entered/deleted
				if (_isInputFriendly) {
					scope.events = {};
					if (!_isContentEditable) {
						// if a textarea or input just add in change and blur
						// handlers, everything else is done by angulars input
						// directive
						...
						element.on('keydown', scope.events.keydown = function(event, eventData) {
							/*
							 * istanbul ignore else: this is for catching the
							 * jqLite testing
							 */
							if (eventData)
							...
							/*
							 * istanbul ignore else: otherwise normal
							 * functionality
							 */
							if (event.keyCode === 9) { // tab was pressed
								...
								if (event.shiftKey) {
									...
									if (_tab !== -1 && _tab >= _linebreak) {
										// set textarea value to: text before
										// caret + tab + text after caret
									...
									// set textarea value to: text before caret
									// + tab + text after caret
									element.val(value.substring(0, start) + "\t" + value.substring(end));

									// put caret at right position again (add
									// one for the tab)
									this.selectionStart = this.selectionEnd = start + 1;
								}
						...
						// add a forEach function that will work on a NodeList,
						// etc..
						...
						var recursiveListFormat = function(listNode, tablevel) {
							...
							forEach(_subnodes, function(index, node) {
								...
								/*
								 * istanbul ignore next: not tested, and this
								 * was original code -- so not wanting to
								 * possibly cause an issue, leaving it...
								 */
								if (!node.outerHTML) {
									// no html to add
									return;
								...
								if (nodeName === 'ul' || nodeName === 'ol') {
									...
									// no reformatting within this subnode, so
									// just do the tabing...
						...
						// <ul><!--comment ul-->
						// <li>Nested Line 1</li>
						// <!--comment between nested lines--><li>Nested Line
						// 2</li>
						// </ul>
						// <li>Test Line 3</li>
						// </ol>
						ngModel.$formatters.unshift(function(htmlValue) {
							...
							//
							// doing this -- which is simpiler -- breaks our
							// tests...
							...
							if (_nodes.length > 0) {
								...
								forEach(_nodes, function(index, node) {
									...
									/*
									 * istanbul ignore next: not tested, and
									 * this was original code -- so not wanting
									 * to possibly cause an issue, leaving it...
									 */
									if (!node.outerHTML) {
										// nothing to format!
										return;
									...
									if (htmlValue.length > 0) {
										// we aready have some content, so drop
										// to a new line
										htmlValue += '\n';
									}
									if (nodeName === 'ul' || nodeName === 'ol') {
										// okay a set of list stuff we want to
										// reformat in a nested way
										htmlValue += '' + recursiveListFormat(node, 0);
									} else {
										// just use the original without any
						...
						/*
						 * istanbul ignore next: phantom js cannot test this for
						 * some reason
						 */
						var processpaste = function(text) {
							/*
							 * istanbul ignore else: don't care if nothing
							 * pasted
							 */
							if (text && text.trim().length) {
								...
								if (text.match(/class=["']*Mso(Normal|List)/i) || text.match(/content=["']*Word.Document/i)) {
									...
									if (text.match(/<[^>]*?(ta-bind)[^>]*?>/)) {
										// entire text-angular or ta-bind has
										// been pasted, REMOVE AT ONCE!!
										...
										// in case of pasting only a span -
										// chrome paste, remove them. THis is
										// just some wierd formatting
										// if we remove the '<span
										// class="Apple-converted-space"> </span>'
										// here we destroy the spacing
								...
								// parse whitespace from plaintext input,
								// starting with preceding spaces that get
								// stripped on paste
						...
						element.on('paste', scope.events.paste = function(e, eventData) {
							...
							if (clipboardData && clipboardData.getData && clipboardData.types.length > 0) {// Webkit
																											// -
																											// get
																											// data
																											// from
								...
								e.preventDefault();
								return false;
							} else {// Everything else - empty editdiv and allow
									// browser to paste content into it, then
									// cleanup
								var _savedSelection = $window.rangy.saveSelection(), _tempDiv = angular.element('<div class="ta-hidden-input" contenteditable="true"></div>');
						...
						element.on('cut', scope.events.cut = function(e) {
							// timeout to next is needed as otherwise the
							// paste/cut event has not finished actually
						...
						element.on('keydown', scope.events.keydown = function(event, eventData) {
							...
							taOptions.keyMappings.forEach(function(mapping) {
								...
								if ((mapping.commandKeyCode === 'UndoKey') || (mapping.commandKeyCode === 'RedoKey')) {
									// this is necessary to fully stop the
									// propagation.
							...
							/*
							 * istanbul ignore next: difficult to test as can't
							 * seem to select
							 */
							...
							if (!_isReadonly) {
								...
								/*
								 * istanbul ignore next: difficult to test as
								 * can't seem to select
								 */
								if (event.keyCode === 13 && !event.shiftKey) {
									...
									// if we are in the last element of a
									// blockquote, or ul or ol and the element
									// is blank
									// we need to pull the element outside of
									// the said type
						...
						element.on('keyup', scope.events.keyup = function(event, eventData) {
							...
							if (!_isReadonly && !BLOCKED_KEYS.test(event.keyCode)) {
								// if enter - insert new taDefaultWrap, if
								// shift+enter insert <br/>
								if (_defaultVal !== '' && event.keyCode === 13) {
									if (!event.shiftKey) {
										// new paragraph, br should be caught
										// correctly
										var selection = taSelection.getSelectionElement();
								...
								if (_defaultVal !== '' && val.trim() === '') {
									...
									/*
									 * we no longer do this, since there can be
									 * comments here and white space var
									 * _savedSelection =
									 * $window.rangy.saveSelection(); val =
									 * _compileHtml(); val = "<" +
									 * attrs.taDefaultWrap + ">" + val + "</" +
									 * attrs.taDefaultWrap + ">";
									 * _setInnerHTML(val);
									 * $window.rangy.restoreSelection(_savedSelection);
									 */
						...
						element.on('blur', scope.events.blur = function() {
							_focussed = false;
							/*
							 * istanbul ignore else: if readonly don't update
							 * model
							 */
							if (!_isReadonly) {
								_setViewValue(undefined, undefined, true);
							} else {
								_skipRender = true; // don't redo the whole
													// thing, just check the
													// placeholder logic
								ngModel.$render();
							}
						});
				...
				var fileDropHandler = function(event, eventData) {
					...
					if (eventData)
						angular.extend(event, eventData);
					// emit the drop event, pass the element, preventing should
					// be done elsewhere
				...
				// changes to the model variable from outside the html/text
				// inputs
				ngModel.$render = function() {
					/*
					 * istanbul ignore if: Catches rogue renders, hard to
					 * replicate in tests
					 */
					...
					// if the editor isn't focused it needs to be updated,
					// otherwise it's receiving user input
					if (!_skipRender) {
						...
						if (_isContentEditable && _focussed) {
							...
							_renderTimeout = $timeout(function() {
								/*
								 * istanbul ignore if: Can't be bothered testing
								 * this...
								 */
								if (!_focussed) {
									element[0].focus();
						...
						if (_isContentEditable) {
							...
							// if in WYSIWYG and readOnly we kill the use of
							// links by clicking
							if (!_isReadonly) {
								_reApplyOnSelectorHandlers();
							...
							// make sure the end user can SEE the html code as a
							// display. This is a read-only display element
				...
				if (attrs.taReadonly) {
					...
					// taReadonly only has an effect if the taBind element is an
					// input or textarea or has contenteditable='true' on it.
					...
					scope.$watch(attrs.taReadonly, function(newVal, oldVal) {
						...
						if (newVal) {
							...
							element.removeClass('ta-readonly');
							// we changed to NOT readOnly mode
							// (taReadonly='false')
							if (element[0].tagName.toLowerCase() === 'textarea' || element[0].tagName.toLowerCase() === 'input') {
								element.removeAttr('disabled');
							} else if (_isContentEditable) {
				...
				// if in WYSIWYG and readOnly we kill the use of links by
				// clicking
	...
	// this global var is used to prevent multiple fires of the drop event.
	// Needs to be global to the textAngular file.
	var dropFired = false;
	var textAngular = angular.module("textAngular", ['ngSanitize', 'textAngularSetup', 'textAngular.factories', 'textAngular.DOM', 'textAngular.validators', 'textAngular.taBind']); // This
																																														// makes
																																														// ngSanitize
																																														// required

	textAngular.config([function() {
		// clear taTools variable. Just catches testing and any other time that
		// this config may run multiple times...
	...
	textAngular.run([function() {
		...
		if (typeof define === 'function' && define.amd) {
			...
			// Ensure that rangy and rangy.saveSelection exists on the window
			// (global scope).
	...
	textAngular.directive("textAngular", ['$compile', '$timeout', 'taOptions', 'taSelection', 'taExecCommand', 'textAngularManager', '$window', '$document', '$animate', '$log', '$q', '$parse', function($compile, $timeout, taOptions, taSelection, taExecCommand, textAngularManager, $window, $document, $animate, $log, $q, $parse) {
		return {
			...
			link : function(scope, element, attrs, ngModel) {
				// all these vars should not be accessable outside this
				// directive
				var _keydown, _keyup, _keypress, _mouseup, _focusin, _focusout, _originalContents, _toolbars, _serial = (attrs.serial) ? attrs.serial : Math.floor(Math.random() * 10000000000000000), _taExecCommand, _resizeMouseDown, _updateSelectedStylesTimeout;
				...
				// get the settings from the defaults and add our specific
				// functions that need to be on the scope
				angular.extend(scope, angular.copy(taOptions), {
					// wraps the selection in the provided tag / execCommand
					// function. Should only be called in WYSIWYG mode.
					wrapSelection : function(command, opt, isSelectableElementTool) {
						if (command.toLowerCase() === "undo") {
							...
							// catch errors like FF erroring when you try to
							// force an undo with nothing done
							_taExecCommand(command, false, opt, scope.defaultTagAttributes);
							if (isSelectableElementTool) {
								// re-apply the selectable tool events
								scope['reApplyOnSelectorHandlerstaTextElement' + _serial]();
							}
							// refocus on the shown display element, this fixes
							// a display bug when using :focus styles to outline
							// the box.
							// You still have focus on the text/html input it
				...
				if (attrs.taDefaultTagAttributes) {
					try {
						// TODO: This should use angular.merge to enhance
						// functionality once angular 1.4 is required
				...
				scope.displayElements = {
					// we still need the hidden input even with a textarea as
					// the textarea may have invalid/old input in it,
				...
				scope.displayElements.popover.on('mousedown', function(e, eventData) {
					...
					if (eventData)
						angular.extend(e, eventData);
					// this prevents focusout from firing on the editor when
					// clicking anything in the popover
					e.preventDefault();
					return false;
				});
				...
				scope.showResizeOverlay = function(_el) {
					...
					_resizeMouseDown = function(event) {
						...
						var mousemove = function(event) {
							...
							// DEFAULT: the aspect ratio is not locked unless
							// the Shift key is pressed.
							//
							// attribute: ta-resize-force-aspect-ratio -- locks
							// resize into maintaing the aspect ratio
							var bForceAspectRatio = (attrs.taResizeForceAspectRatio !== undefined);
							// attribute: ta-resize-maintain-aspect-ratio=true
							// causes the space ratio to remain locked
						...
						oneEvent(_body, 'mouseup', function(event) {
							...
							// at this point, we need to force the model to
							// update! since the css has changed!
							// this fixes bug: #862 - we now hide the popover --
							// as this seems more consitent.
							// there are still issues under firefox, the window
							// does not repaint. -- not sure
							// how best to resolve this, but clicking anywhere
							// works.
				...
				// allow for insertion of custom directives on the textarea and
				// div
				...
				// compile the scope with the text and html elements only - if
				// we do this with the main element it causes a compile loop
				...
				scope.startAction = function() {
					...
					// if rangy library is loaded return a function to reload
				...
				// note that focusout > focusin is called everytime we click a
				// button - except bad support:
				// http://www.quirksmode.org/dom/events/blurfocus.html
				// cascades to displayElements.text and displayElements.html
				// automatically.
				_focusin = function() {
					scope.focussed = true;
				...
				_focusout = function(e) {
					// if we are NOT runnig an action and have NOT focussed
					// again on the text etc then fire the blur events
					if (!scope._actionRunning && $document[0].activeElement !== scope.displayElements.html[0] && $document[0].activeElement !== scope.displayElements.text[0]) {
						...
						_toolbars.unfocus();
						// to prevent multiple apply error defer to next seems
						// to work.
						$timeout(function() {
				...
				// Setup the default toolbar tools, this way allows the user to
				// add new tools like plugins.
				// This is on the editor for future proofing if we find a better
				// way to do this.
				scope.queryFormatBlockState = function(command) {
					// $document[0].queryCommandValue('formatBlock') errors in
					// Firefox if we call this when focussed on the textarea
				...
				scope.switchView = function() {
					...
					if (scope.showHtml) {
						...
						$timeout(function() {
							...
							// [0] dereferences the DOM object from the
							// angular.element
				...
				// changes to the model variable from outside the html/text
				// inputs
				// if no ngModel, then the only input is from inside
				// text-angular
				if (attrs.ngModel) {
					var _firstRun = true;
					ngModel.$render = function() {
						if (_firstRun) {
							// we need this firstRun to set the originalContents
							// otherwise it gets overrided by the setting of
							// ngModel to undefined from NaN
							_firstRun = false;
							// if view value is null or undefined initially and
							// there was original content, set to the original
							// content
							var _initialValue = scope.$parent.$eval(attrs.ngModel);
							if ((_initialValue === undefined || _initialValue === null) && (_originalContents && _originalContents !== '')) {
								// on passing through to taBind it will be
								// sanitised
								ngModel.$setViewValue(_originalContents);
							}
						}
						scope.displayElements.forminput.val(ngModel.$viewValue);
						// if the editors aren't focused they need to be
						// updated, otherwise they are doing the updating
					...
					// if no ngModel then update from the contents of the
					// origional html.
				...
				scope.$on('ta-drop-event', function(event, element, dropEvent, dataTransfer) {
					...
					if (dataTransfer && dataTransfer.files && dataTransfer.files.length > 0) {
						angular.forEach(dataTransfer.files, function(file) {
							// taking advantage of boolean execution, if the
							// fileDropHandler returns true, nothing else after
							// it is executed
							// If it is false then execute the
							// defaultFileDropHandler if the fileDropHandler is
							// NOT the default one
							// Once one of these has been executed wrap the
							// result as a promise, if undefined or variable
							// update the taBind, else we should wait for the
							// promise
							try {
				...
				// the following is for applying the active states to the tools
				// that support it
				scope._bUpdateSelectedStyles = false;
				...
				// loop through all the tools polling their activeState function
				// if it exists
				scope.updateSelectedStyles = function() {
					var _selection;
					/*
					 * istanbul ignore next: This check is to ensure multiple
					...
					// used to update the active state when a key is held down,
					// ie the left arrow
				...
				_keypress = function(event, eventData) {
					...
					scope.$apply(function() {
						if (_toolbars.sendKeyCommand(event)) {
							/*
							 * istanbul ignore else: don't run if already
							 * running
							 */
							if (!scope._bUpdateSelectedStyles) {
								scope.updateSelectedStyles();
				...
				// update the toolbar active states when we click somewhere in
				// the text/html boxed
				_mouseup = function() {
	...
	textAngular.service('textAngularManager', ['taToolExecuteAction', 'taTools', 'taRegisterTool', function(taToolExecuteAction, taTools, taRegisterTool) {
		...
		// All publicly published functions that modify/need to access the
		// toolbar or editor scopes should be in here
		// these contain references to all the editors and toolbars that have
		// been initialised in this app
		var toolbars = {}, editors = {};
		// when we focus into a toolbar, we need to set the TOOLBAR's $parent to
		// be the toolbars it's linked to.
		// We also need to set the tools to be updated to be the toolbars...
		return {
			// register an editor and the toolbars that it is affected by
			registerEditor : function(name, scope, targetToolbars) {
				// targetToolbars are optional, we don't require a toolbar to
				// function
				if (!name || name === '')
				...
				angular.forEach(targetToolbars, function(_name) {
					...
					// if it doesn't exist it may not have been compiled yet and
				...
				editors[name] = {
					...
					// this is a suite of functions the editor should use to
					// update all it's linked toolbars
					editorFunctions : {
						disable : function() {
						...
						unfocus : function() {
							// this should be called when the editor becomes
							// unfocussed
							angular.forEach(_toolbars, function(toolbarScope) {
								toolbarScope.disabled = true;
						...
						updateSelectedStyles : function(selectedElement) {
							// update the active state of all buttons on liked
							// toolbars
							angular.forEach(_toolbars, function(toolbarScope) {
						...
						sendKeyCommand : function(event) {
							// we return true if we applied an action, false
							// otherwise
							var result = false;
						...
						triggerElementSelect : function(event, element) {
							// search through the taTools to see if a match for
							// the tag is made.
							// if there is, see if the tool is on a registered
							// toolbar and not disabled.
							// NOTE: This can trigger on MULTIPLE tools
							// simultaneously.
							...
							// get all valid tools by element name, keep track
							// if one matches the
							...
							angular.forEach(taTools, function(tool, name) {
								if (tool.onElementSelect && tool.onElementSelect.element && tool.onElementSelect.element.toLowerCase() === element[0].tagName.toLowerCase() && (!tool.onElementSelect.filter || tool.onElementSelect.filter(element))) {
									// this should only end up true if the
									// element matches the only attributes
							...
							// if we matched attributes to filter on, then
							// filter, else continue
							...
							// Run the actions on the first visible filtered
							// tool only
			...
			// retrieve toolbars by editor name, largely used by testing suites
			// only
			...
			updateToolsDisplay : function(newTaTools) {
				// pass a partial struct of the taTools, this allows us to
				// update the tools on the fly, will not change the defaults.
				var _this = this;
				angular.forEach(newTaTools, function(_newTool, key) {
					_this.updateToolDisplay(key, _newTool);
				});
			},
			// this function resets all toolbars to their default tool
			// definitions
			...
			// toolkey, toolDefinition are required. If group is not specified
			// will pick the last group, if index isnt defined will append to
			// group
			...
			// this is used when externally the html of an editor has been
			// changed and textAngular needs to be notified to update the model.
			...
			// this is used by taBind to send a key command in response to a
			// special key event
			...
			link : function(scope, element, attrs) {
				...
				var setupToolElement = function(toolDefinition, toolScope) {
					...
					if (toolDefinition && !toolDefinition.display && !toolScope._display) {
						...
						// add the icon to the front of the button if there is
						// content
				...
				// default functions and values to prevent errors in testing and
				// on init
				scope._parent = {
					disabled : true,
					showHtml : false,
				...
				var defaultChildScope = {
					...
					isDisabled : function() {
						// to set your own disabled logic set a function or
						// boolean on the tool called 'disabled'
						return ( // this bracket is important as without it
									// it just returns the first bracket and
									// ignores the rest
						// when the button's disabled function/value evaluates
						// to true
						(typeof this.$eval('disabled') !== 'function' && this.$eval('disabled')) || this.$eval('disabled()') ||
						// all buttons except the HTML Switch button should be
						// disabled in the showHtml (RAW html) mode
				...
				angular.forEach(scope.toolbar, function(group) {
					...
					angular.forEach(group, function(tool) {
						...
						// creates a child scope of the main angularText scope
						// and then extends the childScope with the functions of
						// this particular tool
						// reference to the scope and element kept
						scope.tools[tool] = angular.extend(scope.$new(true), taTools[tool], defaultChildScope, {
							name : tool
						});
						scope.tools[tool].$element = setupToolElement(taTools[tool], scope.tools[tool]);
						// append the tool compiled with the childScope to the
						// group element
				...
				// when forceNew is set to true it will ignore all previous
				// settings, used to reset to taTools definition
				// to reset to defaults pass in taTools[key] as _newTool and
				// forceNew as true, ie `updateToolDisplay(key, taTools[key],
				// true);`
				scope.updateToolDisplay = function(key, _newTool, forceNew) {
					var toolInstance = scope.tools[key];
					if (toolInstance) {
						// get the last toolDefinition, then override with the
						// new definition
						...
						// if tool is defined on this toolbar, update/redo the
						// tool
				...
				scope.addTool = function(key, _newTool, groupIndex, index) {
					scope.tools[key] = angular.extend(scope.$new(true), taTools[key], defaultChildScope, {
						name : key
					});
